  ...
  getMenu: async (req, res) => {
    ...
    try {
      ...
      if (error) {
        ...
        return res.status(500).json({ success: false, error: "Error fetching menu." });
      }
  ...
  addItem: async (req, res) => {
    ...
    try {
      ...
      if (itemError) {
        ...
        return res.status(500).json({ success: false, error: "Error getting menu item." });
      }
      ...
      if (tableError) {
        ...
        return res.status(500).json({ success: false, error: "Error adding menu item to table." });
      }

      console.log(`Successfully added item to Table ${tableInfo.tableNumber}.`);
      ...
      if (updatedError) {
        ...
        return res.status(500).json({ success: false, error: "Error fetching updated table." });
      }

      // Send the updated table to frontend for context storage.
      return res.status(200).json({ success: true, msg: `Added item to table ${tableInfo.tableNumber}.`, table: updatedTable });
    } catch(err) {
      console.error("Error adding item to table.", err);
      return res.status(500).json({ success: false, error: "Error adding item to table." });
    }
  },
  ...
  removeItem: async (req, res) => {
    ...
    try {
      ...
      if (itemIndex === -1) {
        ...
        return res.status(404).json({ success: false, error: "Item not found in table." });
      }
      ...
      if (error) {
        ...
        return res.status(500).json({ success: false, error: "Error removing menu item from table." });
      }

      console.log(`Successfully removed item from Table ${tableInfo.tableNumber}.`);
      ...
      // Send the updated table to frontend for context storage.
      return res.status(200).json({ success: true, msg: `Removed item from table ${tableInfo.tableNumber}.`, table: updatedTable });
    } catch(err) {
      console.log("Error removing item from table.", err);
      return res.status(500).json({ success: false, error: "Error removing item from table." });
    }
  },
  ...
  applyDiscount: async (req, res) => {
    ...
    try {
      ...
      if (error) {
        ...
        return res.status(500).json({ success: false, error: "Error applying discount to table." });
      }
